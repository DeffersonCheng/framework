...
public final class ConfigurationUtil {
	...
	public static Object parseXMLObject(final Class clazz, Resource resource) {
		try {
			...
			log.error(String.format("Fail to transfer Java Object[class: {"	+ clazz.getSimpleName() + "}] from {" + resource+ "}, reason: "), e);
			e.printStackTrace();
	...
	public static Resource[] getSortedResources(String location)  {
		...
		if (resources.length == 0) { 
			...
			log.debug("Get [{" + location + "}] configuration at {"
					+ resources.toString() + "}");
		} catch (IOException e) {
	...
	private static String prepareLocation(String location) {
		if (!location.startsWith("classpath"))  
			location = (new StringBuilder()).append("classpath*:").append(
					location).toString();
		return location;
	}
